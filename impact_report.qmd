...
The IntoValue data set (taken from the dashboard) currently contains n = `r distinct(iv_data, id) |> nrow()` unique trials and n = `r nrow(unique_publications)` unique publications (@fig-articles). Some of the publications only included the DOI in the dataset, even if there was a corresponding PMID, while some publications did not have PMIDs at all, but here we present the data set as is, without trying to enrich for missing PMID values (is this still true?).
...
has_cochrane <- iv_citations |> 
  group_by(cited_work_doi, cited_work_pmid, cited_work_oaid) |> 
...
has_cochrane_pmid <- ivcpm |> 
  group_by(cited_work_pmid, cited_work_doi) |> 
  summarise(has_cochrane_review = any(str_detect(cited_by_doi, cochrane_cd_regex), na.rm =  TRUE),
            n_cochrane = sum(str_detect(cited_by_doi, cochrane_cd_regex), na.rm =  TRUE),
            n_cochrane_unique = sum(str_detect(unique(cited_by_doi_deversioned), cochrane_cd_regex), na.rm = TRUE)) 
